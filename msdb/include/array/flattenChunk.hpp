...
template <typename Ty_>
pBlockItemRangeIterator flattenBlock<Ty_>::getItemRangeIterator(const range& r)
{
...
{
	//return std::make_shared<vItemIterator>(flattenBufferItemIterator(this->cached_->getData(),
	//																 this->desc_->dims_,
	//																 range(this->desc_->getIsp(), this->desc_->getIep()),
	//																 this->itemBitmap_));
	//auto tempIt = std::make_shared<flattenBufferItemIterator<uint8_t>>((uint8_t*)this->cached_->getData(), this->desc_->dims_, 
	//																  range(this->desc_->getIsp(), this->desc_->getIep()), 
	//																  this->itemBitmap_);

	//auto out = std::make_shared<vItemIterator>(flattenBufferItemIterator<Ty_>((Ty_*)this->cached_->getData(), this->desc_->dims_,
	//																		  range(this->desc_->getIsp(), this->desc_->getIep()),
	//																		  this->itemBitmap_));

	//vpItemIterator out = { std::make_shared<flattenBufferItemIterator<Ty_>>((Ty_*)this->cached_->getData(),
	//													   this->desc_->dims_,
	//													   range(this->desc_->getIsp(), this->desc_->getIep()),
	//													   this->itemBitmap_) };
	//return out;
	auto it = std::make_shared<flattenBufferItemIterator<Ty_>>((Ty_*)this->cached_->getData(),
														   this->desc_->dims_,
														   range(this->desc_->getIsp(), this->desc_->getIep()),
														   this->itemBitmap_);
	auto iit = std::static_pointer_cast<iItemIterator<Ty_>>(it);
...
{
	// TODO:: change to range iterator
	//return std::make_shared<flattenBufferItemIterator<Ty_>>(this->cached_->getData(),
	//														this->desc_->dims_,
	//														range(this->desc_->getIsp(), this->desc_->getIep()),
	//														this->itemBitmap_);
	 auto it = std::make_shared<flattenBufferItemIterator<Ty_>>((Ty_*)this->cached_->getData(),
